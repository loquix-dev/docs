...
| `retry` | `retry(attachmentId: string): void` | Re-queues a previously errored attachment. Throws — synchronously, not as a rejected promise, since `retry()` isn't `async` — if the ID was never passed to `add()`. Otherwise a no-op unless the attachment is currently errored: active, queued, and completed attachments can't be retried. Re-runs `provider.validate()` if present; a renewed validation failure leaves the attachment errored without firing any callback. |
