...
- **Timeouts** — a default 60-second signal, composed with the controller's own abort signal (there is no signal you supply), passed to your provider. A provider that forwards it to `fetch` cuts off the whole exchange at sixty seconds, not just the wait for the stream to start.
