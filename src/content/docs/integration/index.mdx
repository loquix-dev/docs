...
Three pages cover the rest of this layer: one on the agent provider interface you implement, one on the agent controller that consumes it, and one on keeping API keys off the client entirely. Read the provider page first if you are connecting a new backend; read the keys page first if you are deciding where a request should originate.
